...
export async function middleware(request: NextRequest) {
  ...
  console.log("DEBUG - middleware JWT_SECRET:", process.env.JWT_SECRET ? "exists (len " + process.env.JWT_SECRET.length + ")" : "undefined");
  ...
  if (token) {
    try {
      ...
      if (parts.length === 3) {
        ...
        console.log("DEBUG - middleware token payload:", payload);
      }
    } catch (e: any) {
