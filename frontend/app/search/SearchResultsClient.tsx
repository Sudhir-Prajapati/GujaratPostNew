...
export default function SearchResultsClient({ initialQuery }: { initialQuery: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      getPublicArticles({ query: initialQuery || undefined, limit: 20 }),
      getPublicArticles({ limit: 20 }),
    ]).then(([searchRes, latestRes]) => {
      if (searchRes && searchRes.articles) {
        setResults(searchRes.articles);
      }
      if (latestRes && latestRes.articles) {
    ...
    <main className="min-h-[60vh] bg-background py-8 md:py-12 select-none">
      <div className="mx-auto max-w-screen-xl px-4">
        ...
        <form onSubmit={submit} className="relative mt-6 max-w-3xl">
          ...
          />
          <button type="submit" className="absolute right-2 top-1/2 h-10 -translate-y-1/2 rounded-lg bg-accent px-5 text-sm font-black text-white hover:bg-red-700 transition-colors">
            {t.searchBtn}
          </button>
        </form>
        ...
        <div className="mb-5 mt-10 border-b border-border pb-4">
          <p className="text-sm font-black text-foreground">
            {initialQuery
              ? results.length > 0
                ? t.resultsFor(initialQuery)
                : t.noFoundNotice(initialQuery)
              : t.defaultSection}
          </p>
          <p className="mt-1 text-xs font-semibold text-muted-foreground">
            {initialQuery
              ...
              : t.articlesCount(results.length)}
          </p>
        </div>
